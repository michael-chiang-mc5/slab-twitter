...
class WordCloud(object):
    ...
    def MCgenerate_from_frequencies(self, frequencies):
        """Create a word_cloud from words and frequencies.
        ...
        for x in frequencies:
            rate_ratio = max ((x[1]['rate1']+1)/(x[1]['rate2']+1) , (x[1]['rate2']+1)/(x[1]['rate1']+1))
        ...
        frequencies = sorted(frequencies, key=lambda x: x[1]['rate_ratio'] , reverse=True)


        frequencies = frequencies[:self.max_words]
        # largest entry will be 1

        max_rate_ratio = float(frequencies[0][1]['rate_ratio'])

        frequencies = [ (word, info['rate_ratio'] / max_rate_ratio, info['color']) for word, info in frequencies ]
        ...
        for word, freq, color_word in frequencies:
        ...
        frequencies = [ (word, freq) for word, freq, color in frequencies ]
